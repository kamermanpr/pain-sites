...
Note: Age and CD4 T-cell count treated as a continuous variables.

Note: Age and CD-4 T-cell count were scaled and centered.

## Process data

Extract and process data on study site, participant ID, CD4 T-cell count, age, sex, and pain definition.

```{r process_glm}
# Select demographic data
demo_logit <- demo[, c('ID', 'Site', 'CD4_recent', 'Sex', 'Age', 'Pain_def')] 

# Process data
data_logit <- data %>% 
    # Join with extracted demographics data (demo_reduced)
    left_join(demo_logit) %>% 
    # Remove upper back (only one outcome -- no pain) and ID
    select(-ID, -Upper_back) 

# Convert data_glm to long format
data_logit.long <- data_logit %>% 
    pivot_longer(cols = -c(CD4_recent, Age, Sex, Site, Pain_def),
                 names_to = 'Pain_site',
                 values_to = 'Pain_present')

# Scale Age and CD4_recent
data_logit.long <- data_logit.long %>% 
...
Alpha threshold for significance was corrected to control the family-wise error rate. The correction was applied to the results of the likelihood ratio tests comparing null models to full models. We chose the conservative Dunn-Bonferroni correction. 

## Run `glmer` model for every body site

Note: p-values reported for the likelihood ratio tests below are uncorrected for multiple comparisons.

```{r checks}
#-- Head --#
head <- data_logit.long %>% 
    # Filter by pain site
...
## Compare models 
anova(null_mod, head_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, shdr_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, chst_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, groin_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, hips_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, neck_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, thrx_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, lmbr_mod, test = 'LRT')
...
## Run `glm` for selected body sites
...
## Compare models 
anova(null_mod, thrt_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, arms_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, elbw_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, wrst_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, abdm_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, back_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, butt_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, legs_mod, test = 'LRT')

## Significant difference between the null and full models 
## (family-wise error correction, alpha = 0.0024)
summary(legs_mod)
...
## Compare models 
anova(null_mod, knee_mod, test = 'LRT')
...
## Compare models 
anova(null_mod, ankl_mod, test = 'LRT')
...
Perform GLMM on selected body sites (head, shoulders, chest, hips, cervical spine, thoracic spine, and lumbosacral spine) to extract fixed effect odds ratios (with 95% CIs) to tabulate and to produce forest plots.

```{r glmm}
# Perform GLMM on each selected body sites
glmm_mods <- data_logit.long %>% 
    filter(complete.cases(.)) %>% 
    ...
    mutate(forest_data = map2(.x = OR, 
                              ...
                              ~ as.data.frame(cbind(.x, .y)) %>% 
                                  ...
                                  mutate(rowname = case_when(
                                      ...
                                      rowname == 'Pain_defChronic' ~ 'Pain (chronic)'
                                      )) %>% 
...
Perform GLM on selected body sites (throat, arms, elbows, wrists and hands, abdomen, buttocks, legs, knees, and ankles and feet) to extract fixed effect odds ratios (with 95% CIs) to tabulate and to produce forest plots.

```{r glm}
# Perform GLM on each selected body sites
glm_mods <- data_logit.long %>% 
    filter(complete.cases(.)) %>% 
...
legs <- glm_mods$forest_plots[[7]] + 
    theme(axis.title.x = element_blank())

knees <- glm_mods$forest_plots[[8]] +
    theme(axis.text.y = element_blank(),
          axis.title.x = element_blank())

ankles.feet <- glm_mods$forest_plots[[9]] +
    theme(axis.text.y = element_blank())

buttocks <- glm_mods$forest_plots[[10]] +
    theme(axis.text.y = element_blank(),
          axis.title.x = element_blank())
...
filter <- data_logit %>% 
    select(-Site, -CD4_recent, -Age, -Sex, -Pain_def) %>% 
    pivot_longer(cols = everything(),
                 names_to = 'Site',
                 values_to = 'Response') %>% 
...
data_geq10 <- data_logit %>% 
    pivot_longer(cols = -c(Site, CD4_recent, Sex, Age, Pain_def),
                 names_to = 'Pain_site',
                 values_to = 'Pain_present') %>% 
...
plot_nofacet <- pubs_nofacet %>% 
    select(-CD4_recent, -Sex, -Age, -Pain_present, -Site, -Pain_def) %>% 
    distinct() %>%
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
...
plot_nofacet.sex <- pubs_nofacet.sex %>% 
    select(-CD4_recent, -Age, -Pain_present, -Site, -Pain_def) %>% 
    distinct() %>%
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
...
#-- Body sites by pain chronicity --#
pubs_nofacet.pain_def <- data_geq10 %>% 
    group_by(Pain_site, Pain_def) %>% 
    nest() %>% 
    # Boostrap data
    mutate(boot = map(.x = data,
                      ~ boot(data = .x[, 'Pain_present'],
...
plot_nofacet.pain_def <- pubs_nofacet.pain_def %>% 
    select(-CD4_recent, -Age, -Pain_present, -Site, -Sex) %>% 
    distinct() %>%
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
    ...
    geom_point(shape = 21,
               ...
               stroke = 1) +
    coord_flip() +
    labs(title = 'Body sites by pain chronicity',
         subtitle = '(Point estimate with 95%CI)',
         y = 'Proportion of participants with pain') +
    scale_y_continuous(limits = c(0, 0.6)) +
    scale_fill_manual(values = c('#FFFFFF', '#000000')) +
...
pubs_nofacet.age <- data_geq10 %>%  
    select(-CD4_recent, -Site, -Sex, -Pain_def) %>% 
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
    mutate(Pain_site = str_replace(Pain_site, 
...
```{r supplementary_fig2, fig.width = 9, fig.height = 8}
# Patchwork
pub_plot <- plot_nofacet + plot_nofacet.sex + plot_nofacet.pain_def +
...
#-- Body sites by pain chronicity --#
pubs_nofacet.pain2 <- data_less10 %>% 
    group_by(Pain_site, Pain_def) %>% 
    nest() %>% 
    # Boostrap data
    mutate(boot = map(.x = data,
...
plot_nofacet.pain2 <- pubs_nofacet.pain2 %>% 
    select(-CD4_recent, -Age, -Pain_present, -Site, -Sex) %>% 
    distinct() %>%
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
...
```{r supplementary_fig4}
# Patchwork
pub_plot2 <- plot_nofacet2 + plot_nofacet.sex2 + plot_nofacet.pain2 +
