...
## Chronic assessing pain in the last 3 months
demo <- demo %>% 
...
Pain in the last week or month (acute) versus pain for the last 3 months (chronic)
...
pain_boot <- pain %>% 
    ...
    # Nest by body region and body site
    group_by(Pain, region, body_site) %>% 
    nest() %>% 
    # Boostrap data
    mutate(boot = map(.x = data,
                      ~ boot(data = .x,
...
Extract and process data on study site, participant ID, CD4 T-cell count, age, sex, and pain chronicity.

```{r process_glm}
# Select demographic data
demo_log <- demo[, c('ID', 'Site', 'CD4_recent', 'Sex', 'Age', 'Pain')] 
...
data_log.long <- data_log %>% 
    pivot_longer(cols = -c(CD4_recent, Age, Sex, Site, Pain),
                 names_to = 'Pain_site',
                 values_to = 'Pain_present')
...
Perform GLMM on selected body sites (head, shoulders, chest, groin, hips, cervical spine, thoracic spine, and lumbosacral spine) to extract fixed effect odds ratios (with 95% CIs) to tabulate and to produce forest plots.
    ...
    mutate(forest_data = map2(.x = OR, 
                              ...
                              ~ as.data.frame(cbind(.x, .y)) %>% 
                                  ...
                                  mutate(rowname = case_when(
                                      ...
                                      rowname == 'PainChronic' ~ 'Pain (chronic)'
                                      )) %>% 
...
filter <- data_log %>% 
    select(-Site, -CD4_recent, -Age, -Sex, -Pain) %>% 
    pivot_longer(cols = everything(),
                 names_to = 'Site',
                 values_to = 'Response') %>% 
...
data_geq10 <- data_log %>% 
    pivot_longer(cols = -c(Site, CD4_recent, Sex, Age, Pain),
                 names_to = 'Pain_site',
                 values_to = 'Pain_present') %>% 
...
plot_nofacet <- pubs_nofacet %>% 
    select(-CD4_recent, -Sex, -Age, -Pain_present, -Site, -Pain) %>% 
    distinct() %>%
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
...
plot_nofacet.sex <- pubs_nofacet.sex %>% 
    select(-CD4_recent, -Age, -Pain_present, -Site, -Pain) %>% 
    distinct() %>%
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
...
pubs_nofacet.pain <- data_geq10 %>% 
    group_by(Pain_site, Pain) %>% 
    nest() %>% 
    # Boostrap data
    mutate(boot = map(.x = data,
                      ~ boot(data = .x[, 'Pain_present'],
...
plot_nofacet.pain <- pubs_nofacet.pain %>% 
...
pubs_nofacet.age <- data_geq10 %>%  
    select(-CD4_recent, -Site, -Sex, -Pain) %>% 
    mutate(Pain_site = str_replace(Pain_site,
                                   pattern = '_',
                                   replacement = ' ')) %>% 
    mutate(Pain_site = str_replace(Pain_site, 
...
# Patchwork
pub_plot <- plot_nofacet + plot_nofacet.sex + plot_nofacet.pain +
